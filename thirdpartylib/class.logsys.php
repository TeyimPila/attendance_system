...
class LS {
  ...
  public static $default_config = array(
     ...
     */
    "basic" => array(
      "company" => "Ameican University of Nigeria",
    ...
    "pages" => array(
       ...
       */
      "login_page" => "/aun_attendance_system/login.php",
      /**
  ...
  public static function login($username, $password, $remember_me = false, $cookies = true){
    ...
    if(self::$db === true){
      ...
      if(self::$config['features']['email_login'] === true){
        $query = "SELECT `instructor_id`, `instructor_password`, `psalt`, `attempt` FROM `". self::$config['db']['table'] ."` WHERE `email`=:login OR `email`=:login ORDER BY `instructor_id` LIMIT 1";
      
        echo 'email log in ON <br>';
        echo self::$config['db']['table'].'<br>';
      }else{
        $query = "SELECT `instructor_id`, `instructor_password`, `psalt`, `attempt` FROM `". self::$config['db']['table'] ."` WHERE `email`=:login ORDER BY `instructor_id` LIMIT 1";
      }
      ...
      if($sql->rowCount() == 0){
        ...
        $us_id = $rows['instructor_id'];
        echo $us_id;
        $us_pass = $rows['instructor_password'];
        echo $us_pass;
        $us_salt = $rows['psalt'];
        $status = $rows['attempt'];
  ...
  public static function register( $id, $password, $other = array() ){
    ...
    if( self::userExists($id) || (isset($other['email']) && self::userExists($other['email'])) ){
      ...
      if( count($other) == 0 ){
        ...
        $sql   = self::$dbh->prepare("INSERT INTO `". self::$config['db']['table'] ."` (`instructor_id`, $columns, `instructor_password`, `psalt`, `attempt`) VALUES(:id, :$colVals, :password, :passwordSalt, '')");
  ...
  public static function userExists($identification){
    self::construct();
    if(self::$config['features']['email_login'] === true){
      $query = "SELECT `instructor_id` FROM `". self::$config['db']['table'] ."` WHERE `email`=:login OR `instructor_id`=:login";
    }else{
      $query = "SELECT `instructor_id` FROM `". self::$config['db']['table'] ."` WHERE `instructor_name`=:login";
    }
    $sql = self::$dbh->prepare($query);
    $sql->execute(array(
